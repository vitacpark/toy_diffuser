...
class ToyReward:
    ...
    def actions_to_states(self, tau0: torch.Tensor) -> torch.Tensor:
        ...
        tau0: [B, D] with D=T*2, or [B,T,2]
        returns: [B,T,2]
        """
        if tau0.dim() == 2:
            B, D = tau0.shape
            assert D == self.horizon_T * 2
            a = tau0.view(B, self.horizon_T, 2)
        else:
            a = tau0
            assert a.shape[1] == self.horizon_T and a.shape[2] == 2
        return torch.cumsum(a, dim=1)

    def per_t_reward(self, tau0: torch.Tensor) -> torch.Tensor:
        s = self.actions_to_states(tau0)
        m_pos = torch.tensor([self.goal_x, self.goal_y], device=s.device, dtype=s.dtype)
        m_neg = -m_pos

        pdf_neg = normal_pdf_isotropic(s, m_neg.view(1, 1, 2), var=self.state_var)
        pdf_pos = normal_pdf_isotropic(s, m_pos.view(1, 1, 2), var=self.state_var)
