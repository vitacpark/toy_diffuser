...
class GaussianDiffusion(nn.Module):
    ...
    def guided_p_sample(self, guide, x, mask, t, clip_denoised=True, repeat_noise=False, cond_idx=0, stack_idx=0, global_put_place=[0., 0.], local_put_place=[0., 0.], pg=False, pg_scale=1.0, guide_step=1):
        ...
        with torch.enable_grad():
            ...
            guide_start = x.shape[-2] // 2
            stack_xy = x[..., guide_start:, 7 + stack_idx * 8 : 9 + stack_idx * 8]
            ...
            if pg:
                ...
                tail = stack_xy.shape[-2]
                mid_xy = np.array(
                    [
                        (local_put_place[0] + 3 * global_put_place[0]) / 4,
                        (local_put_place[1] + 3 * global_put_place[1]) / 4,
                    ]
                )

                mid_xy = torch.from_numpy(mid_xy).reshape((1, 1, 2)).to(device)
                mid_xy = mid_xy.repeat(1, tail, 1)

                local_xy = torch.from_numpy(local_put_place).reshape((1, 1, 2)).to(device)
                local_xy = local_xy.repeat(1, tail, 1)

                global_xy = torch.from_numpy(global_put_place).reshape((1, 1, 2)).to(device)
                global_xy = global_xy.repeat(1, tail, 1)
