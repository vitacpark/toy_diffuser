...
def connect(use_gui=True, shadows=True, color=None, width=None, height=None):
    ...
    if use_gui and not is_darwin():
        display = os.environ.get('DISPLAY', '')
        if not display:
            use_gui = False
            print('No display detected! Using DIRECT mode.')
        else:
            # DISPLAY can be set in containers even when X socket is unavailable.
            # In that case, avoid GUI and run headless.
            display_num = display.split(':')[-1].split('.')[0]
            x_socket = f'/tmp/.X11-unix/X{display_num}' if display_num.isdigit() else ''
            if x_socket and not os.path.exists(x_socket):
                use_gui = False
                print(f'DISPLAY={display} but {x_socket} is missing. Using DIRECT mode.')
    ...
    if use_gui and sim_id < 0:
        print('PyBullet GUI connection failed. Falling back to DIRECT mode.')
